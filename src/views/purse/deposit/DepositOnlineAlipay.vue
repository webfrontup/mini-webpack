	...
	<div class="alipay">
		...
		<div class="content">
			...
			<div class="submit">
				...
				<p>
					温馨提示：单笔存款金额为
					<span>{{baseInfoData.singleMin}}~{{baseInfoData.singleMax}}</span>元
				</p>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...
		handleDeposit() {
			...
			if (
				this.depositMoney > this.baseInfoData.singleMax ||
				this.depositMoney < this.baseInfoData.singleMin
			) {
				this.$toast({
					message: `存款金额为${this.baseInfoData.singleMin}-${
						this.baseInfoData.singleMax
					}`,
					duration: 2000
				});
				return;
...
.alipay {
	...
	.choose-depositMoney {
		background: #fff;
		.sys-depositMoney {
			padding: 0.34667rem /* 26/75 */ 0.4rem /* 30/75 */ 0.34667rem
				/* 26/75 */ 0;
		...
		.in-depositMoney {
			h2 {
				.sys-depositMoney {
					padding: 0.34667rem /* 26/75 */ 0.4rem /* 30/75 */
						0.34667rem /* 26/75 */ 0;
					margin-left: 0.4rem /* 30/75 */;
					display: flex;
					justify-content: space-between;
