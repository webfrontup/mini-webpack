	...
	<div class="lowerReport" ref="lowerReport">
		...
		<div class="tab-content">
			<div class="lower-content">
				<div v-if="list.length > 0" class="lis" v-for="(item,index) in list" :key="index">
					<ul>
						<li>
							<label>会员账号：</label>
							<span class="levelBlue">{{item.account}}</span>
						</li>
						<li>
							<label>推广人数：</label>
							<span class="levelBlue">{{item.num}}人</span>
						</li>
						<li>
							<label>报表营收：</label>
							<span class="levelRed">{{item.win>0 ?'+':''}}{{item.win | currency('',2)}}</span>
						</li>
						<li>
							<label>投注总额：</label>
						...
						<li>
							<label>返佣总额：</label>
							<span>{{item.rebate | currency("",2)}}</span>
						</li>
						<li class="botli">
							<label>最后登录时间：</label>
